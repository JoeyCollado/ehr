...
const Page = () => {
  ...
  // 1. Define a type for your fixed symptom keys
  type Symptom =
    | "Fever"
    | "Chills"
    | "Cough"
  ...
  // 2. Define a type for optional symptoms
  interface OptionalSymptom {
    id: number;
    name: string;
    value: string;
  }

  // 3. Use types in your state
  const [toggles, setToggles] = useState<Record<Symptom, boolean>>({
    Fever: false,
    Chills: false,
    Cough: false,
  ...
  const [diagnoses, setDiagnoses] = useState([
    {
      ...
      description:
        "Fungal pneumonia is a non-contagious lung infection caused by fungal spores. It happens when the spores mix with the air and are inhaled, or when an inactive infection is reactivated.",
      category: "COMMON",
    },
    {
      id: 2,
  ...
  const toggleValue = (symptom: keyof typeof toggles) => {
  ...
  const handleDiagnosisChange = (id: number, field: string, value: string) => {
  ...
  if (!isHydrated) {
    return (
      <div className="min-h-screen bg-[#faf6f6] flex items-center justify-center">
        Loading...
      </div>
    );
  }
          ...
          <div className="w-full max-w-5xl bg-white text-black shadow-lg rounded-lg p-6">
            ...
            <div className="space-y-4 mb-8">
              {symptoms.map((symptom) => (
                <div
                  key={symptom}
                  className="flex items-center justify-between border border-gray-300 rounded p-4"
                >
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      title="symptom"
                      type="checkbox"
                      checked={toggles[symptom as Symptom]}
                      onChange={() => toggleValue(symptom as Symptom)}
                    ...
                    <div
                      className={`w-11 h-6 rounded-full peer ${
                        isEditing
                          ...
                          : "bg-gray-100 peer-checked:bg-gray-400"
                      } peer-focus:outline-none peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all`}
                    ></div>
                  </label>
                </div>
              ))}
                  ...
                  >
                    ...
                    {isEditing ? (
                      <input
                        ...
                        value={symptom.value}
                        onChange={(e) =>
                          handleInputChange(symptom.id, e.target.value)
                        }
